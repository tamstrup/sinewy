...
<p><code>ContextMenu.Shortcut</code> is also available for explicit composition. Theme values are rendered as data attributes and CSS custom properties; ordinary <code>style</code> values and tagged-template extensions continue to work.</p>
...
<p>These options become stable <code>data-*</code> styling hooks and are not forwarded as invalid DOM attributes. The facade also supplies the <code>shortcut</code> convenience, styled indicators, <code>.Shortcut</code>, and an SVG <code>.TriggerIcon</code>.</p>
...
<p>Renders the submenu trigger as a menu item with <code>aria-haspopup=&quot;menu&quot;</code>, synchronized expanded state, and a native popover target. It accepts <code>as</code>, <code>disabled</code>, <code>textValue</code>, and ordinary item event attributes.</p>
...
`;var Oa=d(({},[],{route:e})=>Pa(e({"/":Ja,"/components/:slug":Xa,"/?":Or}))),Pa=d`div
  min-height 100svh
  display grid
...
`,Mn={"context-menu":{status:"Preview",tags:["Popover API","Point anchors","Headless + theme"],summary:"Contextual actions at pointer or keyboard invocation points, backed by the shared menu engine.",preview:Qa,previewHeadings:[{id:"live-example",text:"Live example"}]},dropdown:{status:"API reviewed",tags:["Popover API","CSS anchors","Headless + theme"],summary:"Actions, checkbox and radio choices, nested menus, room-aware placement, and a theme facade.",preview:Za,previewHeadings:[{id:"live-example",text:"Live example"},{id:"theme-preview",text:"Theme preview"}]}};function Ln(e,t){return[za(kr({href:"/"},Sr("S"),d`span`(d`strong`("Sinewy"),d`span`("Documentation"))),Ra(mr(d`h2`("Start here"),br({href:"/",data:{active:t.has("/")||void 0}},"Overview")),mr(d`h2`("Components"),Pt.map(n=>br({href:"/components/"+n.slug,data:{active:t.has("/components/"+n.slug)||void 0}},n.title,d`span`(Mn[n.slug]?.status||"Preview"))))),$a(d`strong`("Independent preview"),"Built for Sin.js with the platform.")),La(e)]}function Ja({},[],{route:e,doc:t}){return t.title("Sinewy \u2014 Documentation"),Ln([jn(e),$n(ja(Cr("Independent components for Sin.js"),d`h1`("Small parts. Native behavior."),d`p`("Sinewy is an independently maintained component library for Sin.js. This site documents each public surface as it becomes useful, testable, and honest about its platform boundaries.")),Pn(d`header`(d`h2`("Where things stand"),d`p`("The status here follows implemented and verified behavior, not a speculative component catalog.")),qa(zn(Rn(d`strong`("Portable reference"),$t("Markdown")),d`p`("Component prose, API tables, and platform limits now come directly from the repository documents."),wr(d`span`({style:{width:"100%"}}))),zn(Rn(d`strong`("Behavior suite"),$t("Green")),d`p`("Browser, server rendering, hydration, submenu, and type declaration coverage are in place."),wr(d`span`({style:{width:"100%"}}))),zn(Rn(d`strong`("Accessibility sign-off"),$t({data:{tone:"manual"}},"Manual")),d`p`("Keyboard behavior is covered; supported-browser and assistive-technology verification remains.")))),Pn(d`header`(d`h2`("Components"),d`p`("Each component page combines its portable contract with live Sin examples and generated navigation.")),Pt.map(n=>Tr({href:"/components/"+n.slug},d`div`(d`h3`(n.title),d`p`(Mn[n.slug]?.summary||n.description)),Ar("\u2192")))),Pn({id:"roadmap"},d`header`(d`h2`("Documentation roadmap"),d`p`("The site itself is the visible checklist for turning implementation work into a usable system.")),Fa(d`li`(zt({data:{done:""}},"\u2713"),d`div`(d`strong`("Establish the documentation shell"),"Navigation, progress overview, component layout, and live examples.")),d`li`(zt({data:{done:""}},"\u2713"),d`div`(d`strong`("Render portable Markdown"),"Frontmatter, GFM content, heading IDs, links, and tables feed the site directly.")),d`li`(zt({data:{current:""}},"3"),d`div`(d`strong`("Generate the static site"),"Use Sin SSR and route discovery to publish every documented component.")),d`li`(zt("4"),d`div`(d`strong`("Expand through real components"),"Add primitives only as their contracts become concrete enough to document.")))))],e)}function Xa({slug:e},[],t){let n=gr[e];if(!n)return t.doc.status(404),Or({},[],t);let o=Mn[e]||{},r=o.preview?o.preview():[],i=o.previewHeadings||[];return t.doc.title(n.title+" \u2014 Sinewy"),Ln([jn(t.route),$n(Dr(Na(d`a`({href:"/"},"Components"),d`span`("/"),d`span`(n.title)),Ba($t(o.status||"Preview"),(o.tags||[]).map(a=>Ga(a))),d`h1`(n.title),d`p`(n.description)),Ua(Ha(r,Va({data:{source:n.source}},d.trust(n.html))),Ka(d`strong`("On this page"),[...i,...n.headings.filter(a=>a.depth===2)].map(a=>d`a`({href:"#"+a.id},a.text)))))],t.route)}function Za(){return[d`section#live-example`(d`h2`("Live example"),d`p`("The themed facade keeps the headless part structure and remains open to normal Sin style extension."),d`div`(Ir(ed()),Er(`import Dropdown from 'sinewy/theme'
...
)`))),d`section#theme-preview`(d`h2`("Theme preview"),d`p`("Size and color establish an inherited menu scope. Parts can make deliberate local overrides."),Wa(vr(d`h3`("Sizes"),xr(Rt({label:"Size 1",size:"1",color:"indigo"}),Rt({label:"Size 2",size:"2",color:"indigo"}),Rt({label:"Size 3",size:"3",color:"indigo"}))),vr(d`h3`("Colors"),xr(...["gray","indigo","cyan","green","amber","crimson","purple"].map(e=>Rt({label:td(e),variant:"soft",color:e}))))))]}function Qa(){return d`section#live-example`(d`h2`("Live example"),d`p`("Right-click or press and hold the target, or focus it and press Shift+F10."),d`div`(Ir(O(_a("Open a contextual menu here"),O.Content({variant:"soft",color:"indigo"},O.Item({shortcut:"\u2318 R"},"Rename"),O.Item({shortcut:"\u2318 D"},"Duplicate"),O.Separator(),O.Item({color:"red"},"Delete")))),Er(`import { ContextMenu } from 'sinewy/theme'
...
ContextMenu(
...
)`)))}var ed=d(()=>{let e=d.live(!0);return()=>E(E.Trigger({variant:"outline",color:"accent",size:"2"},"Open menu",E.TriggerIcon()),E.Content({align:"start",offset:7,variant:"soft",color:"indigo"},E.Label("Workspace"),E.Item({shortcut:"\u2318 E"},"Edit details"),E.Checkbox({bind:e},E.Indicator("\u2713"),"Notifications"),E.Separator(),E.Sub(E.SubTrigger("Share"),E.SubContent(E.Item("Copy link"),E.Item("Invite people")))))});function Rt({label:e,size:t="2",variant:n="solid",color:o,highContrast:r=!1,dark:i=!1}){return Ya({data:{dark:i||null}},d`span`(e),E(E.Trigger({size:t,variant:n==="soft"?"soft":"outline",color:o,highContrast:r},"Open",E.TriggerIcon()),E.Content({size:t,variant:n,color:o,highContrast:r,align:"start",offset:6},E.Item({shortcut:"\u2318 E"},"Edit"),E.Checkbox({checked:!0},E.Indicator("\u2713"),"Enabled"),E.Item({color:"red"},"Delete"))))}function td(e){return e[0].toUpperCase()+e.slice(1)}function jn(e){return d`header
    display none
    align-items center
    justify-content space-between
    gap 12
  ...
  `(kr({href:"/"},Sr("S"),d`strong`("Sinewy")),Ma(yr({href:"/",data:{active:e.has("/")||void 0}},"Overview"),Pt.map(t=>yr({href:"/components/"+t.slug,data:{active:e.has("/components/"+t.slug)||void 0}},t.title))))}function Or({},[],{route:e,doc:t}){return t.title("Not found \u2014 Sinewy"),Ln([jn(e),$n(Dr(Cr("404"),d`h1`("Nothing here yet."),d`p`("This documentation is growing alongside the component system."),Tr({href:"/"},d`div`(d`h3`("Return to the overview"),d`p`("See current progress and available component pages.")),Ar("\u2192"))))],e)}var Qd=d.mount(Oa);export{Qd as default};
